...
async function filterDeprecatedTokens(context: PandaContext, tokens: DeprecatedToken[]): Promise<DeprecatedToken[]> {
...
async function filterInvalidTokens(context: PandaContext, paths: string[]): Promise<string[]> {
...
async function isColorAttribute(context: PandaContext, _attribute: string): Promise<boolean> {
  const category = await getPropertyCategory(context, _attribute)
  return category === 'colors'
}

async function isColorToken(context: PandaContext, value: string): Promise<boolean> {
...
async function isValidFile(context: PandaContext, fileName: string): Promise<boolean> {
  return context.getFiles().some((file) => arePathsEqual(file, fileName))
}

async function isValidProperty(context: PandaContext, name: string, patternName?: string) {
...
async function matchFile(context: PandaContext, name: string, imports: ImportResult[]) {
...
async function resolveLongHand(context: PandaContext, name: string): Promise<string | undefined> {
...
async function resolveShorthands(context: PandaContext, name: string): Promise<string[] | undefined> {
